...
var modeCache = {};
modeCache[wpi.INPUT] = 'INPUT';
modeCache[wpi.OUTPUT] = 'OUTPUT';
modeCache[wpi.PWM_OUTPUT] = 'PWM_OUTPUT';
modeCache[wpi.GPIO_CLOCK] = 'GPIO_CLOCK';
modeCache[wpi.SOFT_PWM_OUTPUT] = 'SOFT_PWM_OUTPUT';
modeCache[wpi.SOFT_TONE_OUTPUT] = 'SOFT_TONE_OUTPUT';
modeCache['IN'] = wpi.INPUT;
modeCache['OUT'] = wpi.OUTPUT;
modeCache['INPUT'] = wpi.INPUT;
modeCache['OUTPUT'] = wpi.OUTPUT;
modeCache['PWM_OUTPUT'] = wpi.PWM_OUTPUT;
modeCache['GPIO_CLOCK'] = wpi.GPIO_CLOCK;
modeCache['SOFT_PWM_OUTPUT'] = wpi.SOFT_PWM_OUTPUT;
modeCache['SOFT_TONE_OUTPUT'] = wpi.SOFT_TONE_OUTPUT;
...
md.modeNumber = function(text){
	var key = text.toUpperCase();
	return modeCache[key];
};

md.modeText = function(number) {
	var key = parseInt(number);
	return modeCache[key];
};

md.modeEncode = function(pinData){
	var cloned = util.clone(pinData);
	cloned.mode = md.modeText(pinData.mode);
	return cloned;
}
md.modeDecode = function(pinData){
	var cloned = util.clone(pinData);
	cloned.mode = md.modeNumber(pinData.mode);
	return cloned;
}

md.pinName = function(pin){
	return pinNameMap[pin];
}

md.pinNum = function(){
	var pi_model = wpi.piBoardId().model;
